...
try:
    ...
    print("[OK] Persistencia PostgreSQL disponivel")
except ImportError as e:
    HAS_PERSISTENCE = False
    print(f"[!] Persistencia nao disponivel: {e}")
...
class AutonomousBot:
    ...
    def __init__(self):
        ...
        # Atributo para controle de persistência
        self.use_persistence = HAS_PERSISTENCE
...
async def main():
    """Função principal com persistência PostgreSQL."""
    try:
        if HAS_PERSISTENCE:
            print(f"\n{Fore.CYAN}{'='*70}")
            print(f"{Fore.CYAN}  INICIALIZANDO BOT COM PERSISTÊNCIA POSTGRESQL")
            print(f"{Fore.CYAN}{'='*70}\n")
            bot = BotWithPersistence()
        else:
            print(f"\n{Fore.YELLOW}{'='*70}")
            print(f"{Fore.YELLOW}  INICIALIZANDO BOT SEM PERSISTÊNCIA")
            print(f"{Fore.YELLOW}{'='*70}\n")
            from bot_master import AutonomousBot as OriginalBot
            bot = OriginalBot()
        ...
        if HAS_PERSISTENCE:
            try:
                await close_repos()
                print(f"{Fore.CYAN}🔌 Conexões PostgreSQL fechadas")
            except Exception as e:
                print(f"{Fore.YELLOW}Aviso: Erro ao fechar conexões: {e}")
